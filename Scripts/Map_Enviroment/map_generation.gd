...
# Range format: Vector2(min_val, max_val)
...
const ground_tile_atlas_coords = [
...
const decoration_tile_atlas_coords = [
...
const tree_tile_atlas_coords = [
	Vector2i(0,0), Vector2i(5,0), Vector2i(11,0), Vector2i(0,10), Vector2i(4,11)
]
...
func generate_chunk(chunk_coords: Vector2i) -> void:
	...
	for y in range(world_pos.y, world_pos.y + generation_size.y):
		for x in range(world_pos.x, world_pos.x + generation_size.x):
			generate_ground(x, y)
			generate_decorations(x, y)
			generate_trees(x, y)
			generate_interactables(x, y)

	loaded_chunks.append(chunk_coords)

func generate_ground(x: int, y: int) -> void:
	var tile = ground_tile_atlas_coords[randi() % ground_tile_atlas_coords.size()]
	ground.set_cell(Vector2i(x, y), 0, tile)

func generate_decorations(x: int, y: int) -> void:
	var val = generation_noise.noise.get_noise_2d(x, y)
	if val >= decor_range.x and val < decor_range.y:
		var decor = decoration_tile_atlas_coords[randi() % decoration_tile_atlas_coords.size()]
		ground_decorations.set_cell(Vector2i(x, y), 0, decor)

func generate_trees(x: int, y: int) -> void:
	var val = generation_noise.noise.get_noise_2d(x, y)
	if val >= tree_range.x and val < tree_range.y:
		var tile = tree_tile_atlas_coords[randi() % tree_tile_atlas_coords.size()]
